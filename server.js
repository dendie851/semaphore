...
const processedTokens = new Set(); // Stores tokens that have been processed (success/fail) to prevent double-process
...
app.post('/api/bayar', async (req, res) => {
    ...
    if (!token) {
        return res.status(403).json({ message: '403 Access Denied: Missing token.' });
    }

    // Check if token was already processed (double-click prevention)
    if (processedTokens.has(token)) {
        return res.status(403).json({ message: 'This payment session has already been processed.' });
    }

    const tokenData = activeTokens.get(token);
    if (!tokenData) {
        return res.status(403).json({ message: '403 Access Denied: Invalid or expired token.' });
    }

    const { userId } = tokenData;

    // Immediately mark token as processed to prevent duplicate requests
    ...
    if (paymentSuccess) {
        if (ticketsAvailable > 0) {
            ...
            return res.json({ status: 'success', message: 'Payment successful!', transactionId, ticketsRemaining: ticketsAvailable });
        } else {
